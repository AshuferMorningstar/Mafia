// --- Auto-refresh Game Room State ---
let gameRoomPollInterval = null;

function startGameRoomPolling() {
    if (gameRoomPollInterval) clearInterval(gameRoomPollInterval);
    if (!roomCode || !playerId) return;
    gameRoomPollInterval = setInterval(() => {
        fetch(`http://127.0.0.1:5001/api/game/${roomCode}/status`)
            .then(res => res.json())
            .then(data => {
...
function handleJoinSuccess(data) {
    ...
    updateLobbyPlayers();
    // Start polling when entering the game room
